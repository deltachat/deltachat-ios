...
class NewGroupController: UITableViewController, MediaPickerDelegate {
    ...
    @objc func doneButtonPressed() {
        ...
        let groupChatId = if createBroadcast {
            dcContext.createBroadcast(name: groupName)
        } else if createEmail {
            dcContext.createGroupChatUnencrypted(name: groupName)
        } else {
            dcContext.createGroupChat(verified: allMembersVerified(), name: groupName)
        }
    ...
    private func showAddMembers(preselectedMembers: Set<Int>) {
        let newGroupController = AddGroupMembersViewController(dcContext: dcContext, preselected: preselectedMembers, isOutBroadcast: createBroadcast, isEmail: createEmail)
