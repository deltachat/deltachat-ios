        ...
        return UIContextMenuConfiguration(
            ...
            actionProvider: { [weak self] _ in
                ...
                let children: [UIMenuElement] = [
                    UIAction.menuAction(localizationKey: muteTitle, systemImageName: muteImage, with: indexPath, action: { self.toggleMute(at: $0) }),
                    UIAction.menuAction(localizationKey: "profile_tag", systemImageName: "tag", with: indexPath, action: { self.setProfileTag(at: $0) }),
                    UIAction.menuAction(localizationKey: "move_to_top", systemImageName: "arrow.up", with: indexPath, action: { self.moveToTop(at: $0) }),
                    UIAction.menuAction(localizationKey: "delete", attributes: [.destructive], systemImageName: "trash", with: indexPath, action: { self.deleteAccount(at: $0) })
                ]
